...
const S={allEmployees:[],actions:[],scenarios:[],elimMsgs:{},players:[],round:0,rng:Math.random,youId:null,traitors:new Set(),analysis:true,difficulty:"Medium",numTraitors:3,log:[],suspicion:{},alive:new Set(),eliminated:new Set(),elimReason:{}};
...
function startGame(){
  S.log=[]; S.round=1; S.players=[]; S.suspicion={}; S.alive=new Set(); S.eliminated=new Set(); S.traitors=new Set(); S.elimReason={};
  ...
  S.players=roster.map(e=>({id:e.id,name:e.name,department:e.department,influence:defaultInfluence(e.department),behaviour:defaultBehaviour(e.department),role:"Innocent",status:"Alive",avatar:`assets/avatars/${e.id}.svg`,avatarSad:`assets/avatars/${e.id}-sad.svg`,votes:0}));
  S.players.forEach(p=>S.alive.add(p.id));
...
function doScenarioPhase(){
  ...
  document.getElementById('answerBtn').onclick=()=>{
    ...
    if(pick===sc.correct){
      ...
      eliminate(S.youId,false,"VotedOut"); renderAll(); checkEnd();
    }
  };
}

// Guaranteed action per bot with fallback
...
function chooseActionFor(playerId, rng){
  ...
  let candidates=[];
  if(isTraitor){ const sabotage=rng()<d.traitor_rate; candidates=[ sabotage ? "traitor_sabotage" : (rng()<.5?"decoy":"safe") ]; }
  else { const err=rng()<d.innocent_error; candidates=[ err ? "risky_innocent" : (rng()<p.behaviour.safe?"safe":"decoy") ]; }
  const fallback=isTraitor?["traitor_sabotage","decoy","safe","risky_innocent","red_herring"]:["safe","decoy","risky_innocent","red_herring","traitor_sabotage"];
  const tryOrder=[...candidates, ...fallback.filter(b=>!candidates.includes(b))];
  for(const b of tryOrder){ const pool=poolBy(b); if(pool.length){ const act=pickRandom(pool,rng); return {act, usedBucket:b, fellBack:(b!==candidates[0])}; } }
  return {act:{description:"…did some uneventful work.",risk_level:0,actually_suspicious:false},usedBucket:"safe",fellBack:true};
}

function doActionsPhase(){
  const aliveIds=[...S.alive]; const items=[];
  aliveIds.filter(id=>id!==S.youId).forEach(id=>{
    const {act,usedBucket,fellBack}=chooseActionFor(id,S.rng);
    const add=act.risk_level*.8 + (act.actually_suspicious?1.2:0)*DIFF[S.difficulty].pattern_clarity;
    S.suspicion[id]=Math.max(0,(S.suspicion[id]||0)*.75 + add);
...
function handlePlayerVote(targetId){
  ...
  const diff=DIFF[S.difficulty]; const aliveIds=[...S.alive]; const voters=aliveIds.filter(id=>id!==S.youId);
  const candidates=aliveIds.filter(id=>id!==S.youId); const planned=[];
  voters.forEach(id=>{
    ...
    else { if(S.rng()<diff.vote_noise){ vote=sorted[Math.floor(S.rng()*Math.max(1,sorted.length))]||baseTarget; } else vote=baseTarget; }
  ...
  let i=0; (function step(){
    if(i<planned.length){
      ...
      Object.entries(tally).forEach(([id,v])=>{ if(v>maxVotes){maxVotes=v; eliminated=id;} else if(v===maxVotes){ if((S.suspicion[id]||0)>(S.suspicion[eliminated]||0)) eliminated=id; } });
      const isTraitor=S.traitors.has(eliminated);
      eliminate(eliminated,isTraitor,"VotedOut"); renderAll(); logLine(`Eliminated: ${nameOf(eliminated)} (${isTraitor?'Traitor':'Innocent'}).`);
      if(!isTraitor){
        const innocents=[...S.alive].filter(id=>id!==S.youId&&!S.traitors.has(id));
        if(innocents.length){
          innocents.sort((a,b)=>{const pa=S.players.find(p=>p.id===a);const pb=S.players.find(p=>p.id===b);return (pb.influence-pa.influence)||((S.suspicion[a]||0)-(S.suspicion[b]||0));});
          const struck=innocents[0];
          setTimeout(()=>{ eliminate(struck,false,"NightStrike"); renderAll(); logLine(`Night strike: ${nameOf(struck)} was eliminated by traitors.`); if(!checkEnd()){S.round+=1;renderRoundInfo();doScenarioPhase();} }, 600);
          return;
        }
      }
...
function renderTopbar(){
  ...
  top.innerHTML=S.players.map(p=>{
    const cls=['player-card']; if(S.eliminated.has(p.id)) cls.push('eliminated');
    if(S.eliminated.has(p.id)){cls.push(S.traitors.has(p.id)?'traitor':'innocent'); cls.push(S.elimReason[p.id]==="NightStrike"?'by-traitors':'by-vote');}
    const tag=p.id===S.youId?`<div class="tag">You</div>`:'';
    const img=S.eliminated.has(p.id)?p.avatarSad:p.avatar;
    return `<div class="${cls.join(' ')}" data-id="${p.id}">${tag}<img src="${img}" alt="${p.name} avatar"><div class="name">${p.name}</div><div class="xmark">✕</div></div>`;
  }).join('');
}
...
window.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('startBtn').onclick=()=>{
    ...
    let seed=0; if(seedStr){seed=0; for(let i=0;i<seedStr.length;i++){seed=((seed<<5)-seed)+seedStr.charCodeAt(i); seed|=0;} if(seed<0)seed=-seed;} else {seed=Math.floor(Math.random()*1e9);}
